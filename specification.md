...
* 目標1：ローカルで動くものを作る
  ...
  * 1-1. バックエンド実装＋CLIで動作確認
    ...
    * CLI は設定コマンドで日本株（デフォルト）または米国株のどちらを扱うか切り替え可能（設定値は SQLite に保存）。市場ごとに共有メモを参照。
  * 1-2. フロントエンド実装
...
* 週次：週初に週の目標を設定 → 各営業日で「ポートフォリオ更新 (`run-daily`) →（市場引け時）→ 反省・共有メモ更新 (`update-memo`)」 → 週末に1週間のまとめ
...
* 入出力
  ...
  * 入力：―
  * 出力：サマリ、活動記録
* 使用ツール：共有メモ取得、資金情報取得、保有銘柄・ウォッチリスト取得、週の目標設定
* 主要機能
...
* 入出力
  ...
  * 入力：1日の目標
  * 出力：サマリ、活動記録
* 使用ツール：共有メモ取得、Explorer、ResearchLeader、Decider 実行、保有・ウォッチ変更、資金・保有取得、活動記録書き込み
* 主要機能
...
* 入出力
  ...
  * 入力：現在の保有・ウォッチ
  * 出力：銘柄コードのリスト、活動記録
* 使用ツール：共有メモ（ナレッジベース）取得、インターネット検索
* 主要機能
...
* 入出力
  ...
  * 入力：銘柄コード
  * 出力：0–1 の点数、理由
* 使用ツール：共有メモ参照、インターネット検索、Grok（X の動向調査）
* 主要機能
...
* 入出力
  ...
  * 入力：活動記録、週の目標
  * 出力：サマリ
* 使用ツール：共有メモ更新、資金情報取得、保有・ウォッチ取得
* 主要機能（CLI からは `update-memo` コマンドで実行）
...
* ナレッジベース：SQLite（`knowledge_documents` テーブル）に市場ごとの単一メモを保存し、Checker が日次で活動サマリ・改善点・要求を集約して更新する。各エージェントは参照して行動に反映する。
